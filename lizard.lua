...
getgenv().ForceHitConnection = getgenv().RunService.RenderStepped:Connect(function()
    ...
    for i, target in ipairs(targets) do
        ...
        if target then
            ...
            if canShoot then
                if autoFire then getgenv().Shoot(target) end
...
local lastClickShot = 0
getgenv().ForceHitClickConnection = getgenv().UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if gameProcessed then return end
    if input.UserInputType ~= Enum.UserInputType.MouseButton1 then return end
    if not Toggles.ForceHitEnabled.Value then return end
    if not (Toggles.FireOnClick and Toggles.FireOnClick.Value) then return end

    local now = tick()
    if now - lastClickShot < 0.1 then return end
    lastClickShot = now

    local targets = {getgenv().ForceHitTarget, getgenv().ForceHitTarget2}
    for _, target in ipairs(targets) do
        if not target then continue end
        if getgenv().isFullyDead(target) then continue end
        if Toggles.KOCheck.Value and getgenv().isKO(target) and not Toggles.StompTarget.Value then continue end
...
pcall(function()
 ...
 getgenv().Library:OnUnload(function()
    ...
    if getgenv().ForceHitClickConnection then getgenv().ForceHitClickConnection:Disconnect() end
