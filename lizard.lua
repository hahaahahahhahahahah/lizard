...
getgenv().Shoot = function(p)
    ...
    local pellets = 5
    if Options.ForceHitPellets then pellets = Options.ForceHitPellets.Value end
    pellets = math.clamp(pellets, 1, 12)
    local hits = {}
    local partData = {}
    for i = 1, pellets do
...
getgenv().GetClosestPlayer = function()
    ...
    for _, plr in ipairs(getgenv().Players:GetPlayers()) do
        if plr ~= getgenv().LocalPlayer and plr.Character and not getgenv().isKO(plr) then
            local part = plr.Character:FindFirstChild(Options.HitPart.Value) or plr.Character:FindFirstChild("HumanoidRootPart")
            if part then
...
getgenv().ForceHit = {}
getgenv().ForceHit.PrevMouseDown = false
...
getgenv().MainGb:AddToggle('ForceHitEnabled', {
    ...
    Callback = function(v)
        ...
        if not getgenv().ForceHitTarget or not getgenv().ForceHitTarget.Character or getgenv().isFullyDead(getgenv().ForceHitTarget) then
            getgenv().ForceHitTarget = nil
            local closest = getgenv().GetClosestToMouse() or getgenv().GetClosestPlayer()
            if closest then
                getgenv().ForceHitTarget = closest
                getgenv().SetupDamageDetection(closest)
                if getgenv().Library.Notify then getgenv().Library:Notify("Target Locked: " .. closest.Name, 1.5) end
            end
        end
    end
...
getgenv().MainGb:AddToggle('AutoRelock', {
    Text = 'Auto Relock Target',
    Default = true,
    Tooltip = 'Automatically lock a new target when the current one dies, KOs, or despawns'
})
getgenv().MainGb:AddSlider('ForceHitPellets', { Text = 'Pellets per shot', Default = 5, Min = 1, Max = 12, Rounding = 0, Tooltip = 'More pellets = more hits per shot' })
getgenv().MainGb:AddToggle('FireOnClick', {
    Text = 'Click to force hit (auto if off)',
    Default = false,
    Tooltip = 'ON = one shot per left click. OFF = fires automatically every frame'
})
...
getgenv().ForceHitConnection = getgenv().RunService.RenderStepped:Connect(function()
    ...
    local autoFire = not (Toggles.FireOnClick and Toggles.FireOnClick.Value)

    local mouseDown = getgenv().UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1)
    local clickFired = autoFire or (mouseDown and not getgenv().ForceHit.PrevMouseDown)
    getgenv().ForceHit.PrevMouseDown = mouseDown
    ...
    local currentTarget = getgenv().ForceHitTarget
    if currentTarget then
        local targetValid = currentTarget.Character
            and currentTarget.Character.Parent
            and not getgenv().isFullyDead(currentTarget)
            and not (Toggles.KOCheck.Value and getgenv().isKO(currentTarget) and not Toggles.StompTarget.Value)
        if not targetValid then
            if Toggles.AutoRelock and Toggles.AutoRelock.Value then
                getgenv().ForceHitTarget = nil
                local now = tick()
                if now - lastClosestScan >= 0.15 then
                    lastClosestScan = now
    ...
    elseif Toggles.AutoRelock and Toggles.AutoRelock.Value then
        local now = tick()
        if now - lastClosestScan >= 0.15 then
            lastClosestScan = now
            local closest = getgenv().GetClosestToMouse() or getgenv().GetClosestPlayer()
...
do
    local function tv(n) local t = Toggles[n] return t and t.Value end
    local DebugGb = getgenv().MainGb
    DebugGb:AddToggle('DebugShootArgs', { Text = 'Debug Shoot Args', Default = false, Tooltip = 'Logs the real "Shoot" args the game sends when you fire normally' })

    local debugHooked = pcall(function()
        local mainEvent = getgenv().MainEvent
        local oldDebugNamecall
        oldDebugNamecall = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
            if tv('DebugShootArgs') then
                local method = getnamecallmethod and getnamecallmethod() or ""
                if self == mainEvent and method == "FireServer" and select(1, ...) == "Shoot" then
                    local ok, caller = pcall(getcallingscript)
                    local fromGame = true
                    if checkcaller then
                        local cok, cval = pcall(checkcaller)
                        if cok and cval then fromGame = false end
                    end
                    warn("[Shoot DEBUG] caller=" .. (ok and tostring(caller) or "?") .. " | fromGame=" .. tostring(fromGame))
                    local dumpArgs = {...}
                    for i = 1, #dumpArgs do
                        local a = dumpArgs[i]
                        warn(("[Shoot DEBUG] arg[%d] type=%s value=%s"):format(i, typeof(a), tostring(a)))
                        if typeof(a) == "table" then
                            for j = 1, #a do
                                local entry = a[j]
                                if typeof(entry) == "table" then
                                    for k, v in pairs(entry) do
                                        warn(("  [%d][%s] = %s (%s)"):format(j, tostring(k), tostring(v), typeof(v)))
                                    end
                                else
                                    warn(("  [%d] = %s (%s)"):format(j, tostring(entry), typeof(entry)))
                                end
                            end
                        end
                    end
            ...
            return oldDebugNamecall(self, ...)
        end))
        getgenv().LizardDebugNamecall = oldDebugNamecall
    end)
    if not debugHooked then
        warn("[Lizard] Debug shoot hook unavailable on this executor")
    end
end
